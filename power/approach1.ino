...
float vpd,vb,vref,vpd2, iL,dutyref,current_mA; // Measurement Variables
...
String dataString;
float vo_max; //higher output voltage limit
float vo_min; //lower output volatge limit
float vd_h=0; //internal signals Vo and 5
float vd_l=0; //internal signals Vo and 4.7
float V_in;
float duty_cycle = 0;
float P_out;
float Output_V;
...
void setup() {
  ...
  if(loopTrigger) { // This loop is triggered, it wont run unless there is an interrupt
    ...
    if (Boost_mode){
      ...
      if (CL_mode) { // Closed Loop Buck
        ...
        vo_max = 5.2;
        vo_min = 4.5; 
        vd_h = Output_V - vo_max;
        vd_l = Output_V - vo_min;
        if (vd_h > 0){
 ...
 dataString = "V_in = " + String( V_in) + ", V_out_pd = " + String( vpd2) + ", V_out = " + String( Output_V) + ", duty_cycle = " + String(duty_cycle) + ", Power_output" + String(P_out);
